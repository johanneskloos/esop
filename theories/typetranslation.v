...
Definition task_map := gmap gname gname.
...
Definition int_type {Σ} := val → task_map → name_map → val → iProp Σ.
Definition int_heap {Σ} := conn_map → task_map → name_map → iProp Σ.
...
Section TypeInterpretation.
  ...
  Implicit Type M: task_map.
  Implicit Type x: val.

  Definition int_i_bool d M N x: iProp Σ := ⌜(x = Ctrue ∨ x = Cfalse) ∧ x = d⌝.
  Definition int_s_bool := int_i_bool.
  Definition int_i_unit d M N x: iProp Σ := ⌜x = Cunit⌝.
  Definition int_s_unit := int_i_unit.
  Definition int_i_ref (ξ: gname) d M N x: iProp Σ := ⌜∃ l: loc, N!!ξ = Some (Loc l) ∧ x = Cloc l⌝.
  Definition int_s_ref := int_i_ref.
  Definition int_s_promise ξ A (τ: type) d M N x: iProp Σ :=
    ∃ γ (T: task_dataC),
      own γ (to_agree T) ∗ 
          ⌜∃ t, N!!ξ = Some (Task t) ∧ M!!ξ = Some γ ∧ x = Ctid t ∧
                td_M_pre T ≡[names τ ∖ A]≡ M⌝.

  (** We build the implementation interpretation of promise in stages. *)
  (** Access to the task data. *)
  Definition ip_data γ U (D': conn_map) (M': task_map) (N': name_map) :=
    own γ (to_agree U) ∗ own (td_D_name U) (agreed D') ∗ own (td_N_name U) (agreed N') ∗
        own (td_M_name U) (agreed M').
  
  (** The easy parts: name map and implementation interpretation. *)
  Definition ip_impl (A: gset gname) (τ: type) (τi: @int_type Σ) (D': conn_map) M' N' M N y :=
    ∃ d, ⌜D' !! RET = Some d ∧ N' ≡[ names τ ∖ A ]≡ N ∧ M' ≡[ names τ ∖ A]≡ M⌝ ∗ τi d M' N' y.
  Definition ip_sim iτs iηs A (τ: type) U D' M': iProp Σ :=
    ∀ N N' σ (eqN: N ≡[(names τ ∪ names (td_post U)) ∖ td_alloc U]≡ N') (eqA: td_alloc U = A),
      existential_triple ⊤ initial_cfg
                         (intΓ iτs (td_env U) (td_D_pre U) (td_M_pre U) N' σ ∗
                               iηs (td_pre U) (td_D_pre U) (td_M_pre U) N')
                         (subst (of_val <$> σ) (td_expr U))
                         (λ v, ∃ d N'', ⌜N ≡[not_new (td_alloc U)]≡ N'' ∧ D' !! RET = Some d⌝ ∧
                                        iτs τ d M' N'' v ∗ iηs (td_post U) D' M' N'').
  (** Put together the inner pieces of the interpretation of promises... *)
  Definition promiseN := nroot .@ "promise".
  Definition int_i_promise_body iτs iηs M N A τ τi γ v :=
    ∃ U D' M' N',
      ip_data γ U D' M' N' ∗ ip_impl A τ τi D' M' N' M N v ∗ □ip_sim iτs iηs A τ U D' M'.
  Definition int_i_promise iτs iηs ξ A τ τi d M N x :=
    ∃ t γ, ⌜N !! ξ = Some (Task t) ∧ M !! ξ = Some γ ∧ x = Ctid t⌝ ∧
           inv promiseN (wp_wait t (int_i_promise_body iτs iηs (delete ξ M) (delete ξ N) A τ τi γ)).
...
Section HeapInterpretation.
  ...
  Definition int_i_emp D M N: iProp Σ := True.
  Definition int_s_emp := int_i_emp.
  Definition int_i_star (iη iη': int_heap) D M N: iProp Σ := iη D M N ∗ iη' D M N.
  Definition int_s_star := int_i_star.
  Definition int_i_pt ξ (iτ: int_type) D M N: iProp Σ :=
    ∃ l d v, ⌜N!!ξ = Some (Loc l) ∧ D!!name ξ = Some d⌝ ∗ mapstoI l v 1%Qp ∗ iτ d M N v.
  Definition int_s_pt ξ (iτ: int_type) D M N: iProp Σ :=
    ∃ l d v, ⌜N!!ξ = Some (Loc l) ∧ D!!name ξ = Some d⌝ ∗ mapstoS l v 1%Qp ∗ iτ d M N v.
  Definition int_i_wait_body γ (η: types.heap) (iη: int_heap) A M N :=
    ∃ U D' N' M', ip_data γ U D' M' N' ∗ ⌜N ≡[ names η ∖ A]≡ N' ∧ M ≡[names η ∖ A]≡ M'⌝ ∗ iη D' M' N'.
  Definition int_i_wait ξ A η (iη: int_heap) D M N: iProp Σ :=
    ∃ t γ, ⌜N!!ξ = Some (Task t) ∧ M!!ξ = Some γ⌝ ∗
                                               wp_wait t (λ _, int_i_wait_body γ η iη A (delete ξ M)
                                                                               (delete ξ N)).
  (* Use the lower-level interpretations here *)
  Definition int_s_wait_pre iτs' (iηs': types.heap → int_heap) U σ: iProp Σ :=
    ∃ N, intΓ iτs' (td_env U) (td_D_pre U) (td_M_pre U) N σ ∗
              iηs' (td_pre U) (td_D_pre U) (td_M_pre U) N.
  Definition int_s_wait iτs' iηs' ξ A η D M N :=
    ∃ t γ U σ,
      ⌜N!!ξ = Some (Task t) ∧ M!!ξ = Some γ ∧
      td_post U = η ∧ td_alloc U = A ∧
      td_M_pre U ≡[names η ∖ A]≡ M⌝ ∧ own γ (to_agree U) ∗
      t ⤇ run: (subst (of_val <$> σ) (td_expr U)) ∗ int_s_wait_pre iτs' iηs' U σ.
...
Section Interpretations.
  ...
  Fixpoint int_s_heap_rec n η :=
    int_s_heap_approx (match n with
                      ...
                      η.
  Definition int_s_heap η: @int_heap Σ := λ D M N, ∃ n, int_s_heap_rec n η D M N.
  ...
  Global Instance int_s_type_persistent τ d M N v:  PersistentP (int_s_type τ d M N v).
  Proof. destruct τ; apply _. Qed.
  Global Instance int_i_type_persistent τ d M N v:  PersistentP (int_i_type τ d M N v).
  Proof. destruct τ; apply _. Qed.
  Global Instance int_s_env_persistent Γ D M N σ: PersistentP (int_s_env Γ D M N σ).
  Proof. apply _. Qed.
  Global Instance int_i_env_persistent Γ D M N σ: PersistentP (int_i_env Γ D M N σ).
  Proof. apply _. Qed.

  Global Instance int_s_type_local τ:
    Proper ((=) ==> overlap (names τ) ==> overlap (names τ) ==> (=) ==> (⊣⊢)) (int_s_type τ).
  Proof.
    induction τ; intros d ? <- M M' eqM N N' eqN x ? <-; cbn; try done.
    ...
    - rewrite /int_s_promise.
      f_equiv; intro γ.
      f_equiv; intro T.
      do 3 f_equiv; intro t.
      rewrite (eqM ξ) ?(eqN ξ).
      2,3: by apply elem_of_union_l, elem_of_singleton.
      do 3 f_equiv.
  ...
  Global Instance int_i_type_local τ:
    Proper ((=) ==> overlap (names τ) ==> overlap (names τ) ==> (=) ==> (⊣⊢)) (int_i_type τ).
  Proof.
    induction τ; intros d ? <- M M' eqM N N' eqN x ? <-; cbn; try done.
    ...
    - rewrite /int_i_promise /int_i_promise_body /ip_impl.
      ...
      rewrite (eqM ξ); last set_solver.
      do 24 f_equiv.
      all: apply (overlap_iff ((names (ttask ξ A τ)))).
      1,3: apply union_subseteq_r.
      all: by apply overlap_delete_proper.
  Qed.
  ...
  Lemma int_s_heap_approx_local
        iη (iη_proper: ∀ (η: types.heap),
               Proper (overlap (names_heap η) ==> overlap (names η) ==> overlap (names η) ==> (⊣⊢))
                      (iη η)) η:
    Proper (overlap (names_heap η) ==> overlap (names η) ==> overlap (names η) ==> (⊣⊢))
           (int_s_heap_approx iη η).
  Proof.
    induction η; intros D D' eqD M M' eqM N N' eqN; cbn.
    - done.
    - rewrite /int_s_star /int_i_star.
      f_equiv; [apply IHη1|apply IHη2].
      all: eapply overlap_mono; eauto.
      all: cbn.
      1,4: rewrite names_heap_star.
      1,3,4: apply union_subseteq_l.
      all: apply union_subseteq_r.
    - rewrite /int_s_pt.
      ...
      enough (∀ d v, int_s_type τ d M N v ⊣⊢ int_s_type τ d M' N' v) as eqint.
      { by setoid_rewrite eqint. }
      intros; apply int_s_type_local; auto.
      all: eapply overlap_mono; eauto.
      all: intros ??; apply elem_of_union; auto.
    - rewrite /int_s_wait /=.
      do 10 f_equiv.
      rewrite (eqN ξ); last set_solver.
      rewrite (eqM ξ); last set_solver.
      do 4 f_equiv.
      all: eapply overlap_iff; last done.
      all: apply union_subseteq_r.
  Qed.
  ...
  Lemma int_s_heap_local η:
    Proper (overlap (names_heap η) ==> overlap (names η) ==> overlap (names η) ==> (⊣⊢))
           (int_s_heap η).
  Proof.
    intros D D' M M' eqM eqD N N' eqN.
    rewrite /int_s_heap.
    f_equiv; intro n.
  ...
  Proof.
    ...
    - rewrite /int_i_pt.
      ...
      assert (M ≡[ names τ ]≡ M') as eqM'.
      { intros ξ' inξ; apply eqM; set_solver. }
      enough (∀ d v, int_i_type τ d M N v ⊣⊢ int_i_type τ d M' N' v) as eqτ.
      { by setoid_rewrite eqτ. }
      intros; by apply int_i_type_local.
    - rewrite /int_i_wait /int_i_wait_body.
      rewrite (eqN ξ); last set_solver.
      rewrite (eqM ξ); last set_solver.
      enough (∀ M'' N'', (delete ξ N ≡[ names η ∖ A ]≡ N'' ∧ delete ξ M ≡[ names η ∖ A ]≡ M'')
                           ↔ (delete ξ N' ≡[ names η ∖ A ]≡ N'' ∧ delete ξ M' ≡[ names η ∖ A ]≡ M''))
        as eqMN'.
      { by setoid_rewrite eqMN'. }
      intros.
      rewrite -!(symmetry_iff (overlap (names η ∖ A)) N'').
      rewrite -!(symmetry_iff (overlap (names η ∖ A)) M'').
      f_equiv.
      all: apply (overlap_iff (names (hwait ξ A η))).
      1,3: apply union_subseteq_r.
      all: by apply overlap_delete_proper.
  Qed.

  Lemma intΓ_local (ϕ: type → @int_type Σ)
        (ϕ_local: ∀ τ, Proper ((=) ==> overlap (names τ) ==> overlap (names τ) ==> (=) ==> (⊣⊢))
                              (ϕ τ)) Γ:
    Proper (overlap (names_env Γ) ==> overlap (names Γ) ==> overlap (names Γ) ==> (=) ==> (⊣⊢))
           (intΓ ϕ Γ).
  Proof.
    intros D D' eqD M M' eqM N N' eqN σ ? <-.
    rewrite /intΓ.
    ...
    2,3,4,6,7,8: done.
    all: intro ξ; rewrite env_elem_of_names; exists x,τ; eauto.
  Qed.
  ...
  Lemma int_s_heap_approx_mono ϕ ϕ' (mono: ∀ η D M N, ϕ η D M N -∗ ϕ' η D M N) η:
    ∀ D M N, int_s_heap_approx ϕ η D M N -∗ int_s_heap_approx ϕ' η D M N.
  Proof.
    induction η; cbn.
    - iIntros (???) "$".
    - iIntros (D M N) "[I1 I2]".
    ...
    - iIntros (D M N).
      ...
      iApply (mono with "ϕ").
  Qed.
  Lemma int_s_heap_rec_S n: ∀ η D M N, int_s_heap_rec n η D M N -∗ int_s_heap_rec (S n) η D M N.
  Proof.
    rewrite -Nat.add_1_r.
    induction n as [|n IH]; cbn.
    all: iIntros (η D M N) "pre"; iApply (int_s_heap_approx_mono with "pre").
    - iIntros (????) "[]".
    - exact IH.
  Qed.
  
  Lemma int_s_heap_rec_mono n n' (len: n ≤ n'):
    ∀ η D M N, int_s_heap_rec n η D M N -∗ int_s_heap_rec n' η D M N.
  Proof.
    induction len.
    { iIntros (????) "$". }
    iIntros (η D M N) "pre".
